...
if (_plannerGrid) {
  ...
  function _addActWidgets() {
    ...
    _plannerGrid.querySelectorAll('.day-header').forEach(hdr => {
      ...
      btn.textContent = act.type === 'workout' ? '💪 Workout': act.type === 'steps'? '🚶 Steps'
                      : '+ Activity';
  ...
  const UNIT_GROUPS = [
    ...
    },
    {
      ...
      units: [
        ...
        { label: 'katori (lg)',  grams: 280,  step: 0.5,  hint: 'katori'    },{ label: 'glass',        grams: 240,  step: 0.5,  hint: 'glasses'   },
...
(function () {
  ...
  _css.textContent = `
    .grocery-view-toggle { display:flex; gap:6px; margin-bottom:12px; }
    .grocery-view-btn {
  ...
  const ING_CATEGORY = {
    'mixed vegetables':'🥬 Vegetables','spinach (palak)':'🥬 Vegetables','bottle gourd (sorakaya)':'🥬 Vegetables',
    'ridge gourd (beerakaya)':'🥬 Vegetables','ivy gourd (dondakaya)':'🥬 Vegetables','brinjal (vankaya)':'🥬 Vegetables',
    'bhindi (okra / bendakaya)':'🥬 Vegetables','yellow cucumber (dosakaya)':'🥬 Vegetables',
    'fenugreek leaves (methi)':'🥬 Vegetables','gongura leaves':'🥬 Vegetables','green chilli (large)':'🥬 Vegetables',
    'moong dal sprouts':'🥬 Vegetables','sweet potato':'🥬 Vegetables','sweet corn':'🥬 Vegetables',
    'spring onion':'🥬 Vegetables','green beans':'🥬 Vegetables','green peas':'🥬 Vegetables',
    'cauliflower':'🥬 Vegetables','capsicum':'🥬 Vegetables','mushroom':'🥬 Vegetables','pumpkin':'🥬 Vegetables',
    'beetroot':'🥬 Vegetables','cucumber':'🥬 Vegetables','coriander':'🥬 Vegetables','mint leaves':'🥬 Vegetables',
    'curry leaves':'🥬 Vegetables','green chilli':'🥬 Vegetables','ginger':'🥬 Vegetables','garlic':'🥬 Vegetables',
    'cabbage':'🥬 Vegetables','tomato':'🥬 Vegetables','onion':'🥬 Vegetables','potato':'🥬 Vegetables',
    'carrot':'🥬 Vegetables','spinach':'🥬 Vegetables','palak':'🥬 Vegetables','corn':'🥬 Vegetables',
    'jackfruit':'🥬 Vegetables','raw mango':'🥬 Vegetables','lettuce':'🥬 Vegetables',
    'toor dal (kandi pappu)':'🌾 Grains & Dal','urad dal (minapappu)':'🌾 Grains & Dal',
    'moong dal (pesara pappu)':'🌾 Grains & Dal','masoor dal (red lentil)':'🌾 Grains & Dal',
    'rajma (kidney beans)':'🌾 Grains & Dal','chickpeas (kabuli chana)':'🌾 Grains & Dal',
    'black chickpeas (kala chana)':'🌾 Grains & Dal','chickpeas (kadala)':'🌾 Grains & Dal',
    'white peas (ragda)':'🌾 Grains & Dal','chana (roasted)':'🌾 Grains & Dal',
    'maida (all-purpose flour)':'🌾 Grains & Dal','besan (gram flour)':'🌾 Grains & Dal',
    'poha (flattened rice)':'🌾 Grains & Dal','sabudana (sago)':'🌾 Grains & Dal',
    'foxtail millet (korralu)':'🌾 Grains & Dal','little millet (samalu)':'🌾 Grains & Dal',
    'barnyard millet (udalu)':'🌾 Grains & Dal','rava (sooji)':'🌾 Grains & Dal',
    'basmati rice':'🌾 Grains & Dal','brown rice':'🌾 Grains & Dal','red rice':'🌾 Grains & Dal',
    'rice flour':'🌾 Grains & Dal','wheat flour':'🌾 Grains & Dal','multigrain flour':'🌾 Grains & Dal',
    'ragi flour':'🌾 Grains & Dal','jowar flour':'🌾 Grains & Dal','bajra flour':'🌾 Grains & Dal',
    'kodo millet':'🌾 Grains & Dal','moong dal':'🌾 Grains & Dal','chana dal':'🌾 Grains & Dal',
    'toor dal':'🌾 Grains & Dal','urad dal':'🌾 Grains & Dal','rice':'🌾 Grains & Dal','oats':'🌾 Grains & Dal',
    'bread':'🌾 Grains & Dal','pav bun':'🌾 Grains & Dal','burger bun':'🌾 Grains & Dal',
    'pita bread':'🌾 Grains & Dal','roti / wrap':'🌾 Grains & Dal','pasta':'🌾 Grains & Dal',
    'noodles':'🌾 Grains & Dal','pizza base':'🌾 Grains & Dal','puri (golgappa)':'🌾 Grains & Dal',
    'puffed rice':'🌾 Grains & Dal','vermicelli (semiya)':'🌾 Grains & Dal',
    'vermicelli / rice':'🌾 Grains & Dal','papdi':'🌾 Grains & Dal','sev':'🌾 Grains & Dal',
    'prawns (royyalu)':'🍗Meat & Seafood','fish (chepalu)':'🍗 Meat & Seafood',
    'minced chicken (keema)':'🍗 Meat & Seafood','minced meat (keema)':'🍗 Meat & Seafood',
    'chicken breast':'🍗 Meat & Seafood','chicken thigh':'🍗 Meat & Seafood','chicken wings':'🍗 Meat & Seafood',
    'fish fillet':'🍗 Meat & Seafood','chicken':'🍗 Meat & Seafood','mutton':'🍗 Meat & Seafood',
    'prawns':'🍗 Meat & Seafood','fish':'🍗 Meat & Seafood',
    'coconut milk':'🥚 Dairy & Eggs','curd (low-fat)':'🥚 Dairy & Eggs','paneer (low-fat)':'🥚 Dairy & Eggs',
    'milk powder':'🥚 Dairy & Eggs','buttermilk':'🥚 Dairy & Eggs','paneer':'🥚 Dairy & Eggs',
    'cheese':'🥚 Dairy & Eggs','cream':'🥚 Dairy & Eggs','butter':'🥚 Dairy & Eggs','ghee':'🥚 Dairy & Eggs',
    'curd':'🥚 Dairy & Eggs','milk':'🥚 Dairy & Eggs','eggs':'🥚 Dairy & Eggs','egg':'🥚 Dairy & Eggs',
    'boondi':'🥚 Dairy & Eggs',
    'custard apple (sitaphal)':'🍎 Fruits','tender coconut':'🍎 Fruits','mixed fruits':'🍎 Fruits',
    'pomegranate':'🍎 Fruits','strawberry':'🍎 Fruits','pineapple':'🍎 Fruits','watermelon':'🍎 Fruits',
    'mosambi':'🍎 Fruits','coconut':'🍎 Fruits','tamarind':'🍎 Fruits','banana':'🍎 Fruits',
    'orange':'🍎 Fruits','papaya':'🍎 Fruits','grapes':'🍎 Fruits','chikoo':'🍎 Fruits','guava':'🍎 Fruits',
    'mango':'🍎 Fruits','apple':'🍎 Fruits','dates':'🍎 Fruits','lemon':'🍎 Fruits','lime':'🍎 Fruits',
    'kiwi':'🍎 Fruits','pear':'🍎 Fruits',
    'mustard powder':'🌶️ Spices & Condiments','red chilli powder':'🌶️ Spices & Condiments',
    'cumin powder':'🌶️ Spices & Condiments','cumin seeds':'🌶️ Spices & Condiments',
    'mustard seeds':'🌶️ Spices & Condiments','sesame seeds':'🌶️ Spices & Condiments',
    'whole spices':'🌶️ Spices & Condiments','andhra spices':'🌶️ Spices & Condiments',
    'chole masala':'🌶️ Spices & Condiments','sambar powder':'🌶️ Spices & Condiments',
    'rasam powder':'🌶️ Spices & Condiments','tandoori masala':'🌶️ Spices & Condiments',
    'kadai masala':'🌶️ Spices & Condiments','chettinad masala':'🌶️ Spices & Condiments',
    'pav bhaji masala':'🌶️ Spices & Condiments','dabeli masala':'🌶️ Spices & Condiments',
    'schezwan sauce':'🌶️ Spices & Condiments','garlic chutney':'🌶️ Spices & Condiments',
    'garlic sauce':'🌶️ Spices & Condiments','tomato sauce':'🌶️ Spices & Condiments',
    'mint chutney':'🌶️ Spices & Condiments','tamarind chutney':'🌶️ Spices & Condiments',
    'black pepper':'🌶️ Spices & Condiments','baking soda':'🌶️ Spices & Condiments',
    'cornflour':'🌶️ Spices & Condiments','breadcrumbs':'🌶️ Spices & Condiments',
    'soy sauce':'🌶️ Spices & Condiments','olive oil':'🌶️ Spices & Condiments',
    'coconut oil':'🌶️ Spices & Condiments','cardamom':'🌶️ Spices & Condiments',
    'turmeric':'🌶️ Spices & Condiments','saffron':'🌶️ Spices & Condiments',
    'chutneys':'🌶️ Spices & Condiments','mayonnaise':'🌶️ Spices & Condiments',
    'jaggery':'🌶️ Spices & Condiments','spices':'🌶️ Spices & Condiments',
    'sugar':'🌶️ Spices & Condiments','salt':'🌶️ Spices & Condiments','yeast':'🌶️ Spices & Condiments',
    'basil':'🌶️ Spices & Condiments','oil':'🌶️ Spices & Condiments','red chilli':'🌶️ Spices & Condiments',
    'chocolate powder':'📦 Packaged & Other','rooh afza syrup':'📦 Packaged & Other',
    'makhana (fox nuts)':'📦 Packaged & Other','dry fruits mix':'📦 Packaged & Other',
    'dry fruits':'📦 Packaged & Other','pumpkin seeds':'📦 Packaged & Other','mixed nuts':'📦 Packaged & Other',
    'chia seeds':'📦 Packaged & Other','protein bar':'📦 Packaged & Other','dark chocolate':'📦 Packaged & Other',
    'ice cream':'📦 Packaged & Other','rose syrup':'📦 Packaged & Other','basil seeds':'📦 Packaged & Other',
    'soda water':'📦 Packaged & Other','coffee powder':'📦 Packaged & Other','tea leaves':'📦 Packaged & Other',
    'biscuits':'📦 Packaged & Other','peanuts':'📦 Packaged & Other','cashews':'📦 Packaged & Other',
    'almonds':'📦 Packaged & Other','walnuts':'📦 Packaged & Other','honey':'📦 Packaged & Other',
    'papad':'📦 Packaged & Other',};

  const CAT_ORDER = ['🥬 Vegetables','🌾 Grains & Dal','🍗 Meat & Seafood','🥚 Dairy & Eggs','🍎 Fruits','🌶️ Spices & Condiments','📦 Packaged & Other'];

  function _getCategory(ing) {
    const lower = ing.toLowerCase();
    let bestKey = '', bestCat = '📦 Packaged & Other';
    for (const [key, cat] of Object.entries(ING_CATEGORY)) {
      if (lower.includes(key) && key.length > bestKey.length) { bestKey = key; bestCat = cat; }
    }
    return bestCat;
  }
  ...
  function _getIngredients(dishName) {
    ...
    for (const [key, ings] of Object.entries(FOOD_INGREDIENTS)) {
      if (stripped.toLowerCase().startsWith(key.toLowerCase()) || key.toLowerCase().startsWith(stripped.toLowerCase())) return ings;
    }
    return [stripped];
  }
  ...
  function _buildShoppingList() {
    ...
    days.forEach(day => meals.forEach(meal => {
      (plan[day]?.[meal] || []).forEach(item => { _getIngredients(item.name).forEach(ing => ingSet.add(ing)); });
    }));
    const byCat = {};
    ingSet.forEach(ing => { const cat = _getCategory(ing); (byCat[cat] = byCat[cat] || []).push(ing); });
    return byCat;
  }

  let _shopChecked = {};
  function _loadSC() { try { _shopChecked = JSON.parse(localStorage.getItem('wmp_shop_ck')||'{}'); } catch(e){_shopChecked={};} }
  function _saveSC() { localStorage.setItem('wmp_shop_ck', JSON.stringify(_shopChecked)); }
  ...
  function _renderShopping(container) {
    ...
    _rendering = true;
    _loadSC();
    const byCat = _buildShoppingList();
    ...
    container.innerHTML = cats.map(cat => `
      <div class="grocery-category">
        <h3>${cat}</h3>
        ${byCat[cat].sort().map(ing => `
          <div class="grocery-item ${_shopChecked[ing]?'checked':''}">
            <input type="checkbox" ${_shopChecked[ing]?'checked':''} data-ing="${ing.replace(/"/g,'&quot;')}">
            <span class="grocery-item-name">${ing}</span></div>
        `).join('')}
      </div>
    `).join('');
    container.querySelectorAll('input[type=checkbox]').forEach(cb => {
      cb.addEventListener('change', () => {
        _shopChecked[cb.dataset.ing] = cb.checked;
        _saveSC();
  ...
  function _injectToggle() {
    ...
    bar.querySelectorAll('.grocery-view-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (_shoppingMode) { _renderShopping(_gc); } else { if (typeof renderGrocery === 'function') renderGrocery(); }
      });
    });
  }
  ...
  if (_clrBtn) {
    _clrBtn.addEventListener('click', () => {
      if (_shoppingMode) { _shopChecked = {}; _saveSC(); setTimeout(() => _renderShopping(_gc), 80); }
    });
  }
