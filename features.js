...
// ── FEATURE 5: UNIT SELECTOR ──────────────────────────────────────────
(function () {
  const foodList = document.getElementById('foodList');
  if (!foodList) return;

  const UNIT_GROUPS = [
    { group: 'Serving', units: [{ label: 'srv', fixed: 1, step: 0.25, hint: 'servings' }] },
    { group: 'Spoons',  units: [{ label: 'tsp', grams: 5, step: 0.5, hint: 'tsp' }, { label: 'tbsp', grams: 13, step: 0.5, hint: 'tbsp' }] },
    { group: 'Cups',    units: [{ label: 'cup', grams: 187, step: 0.25, hint: 'cups' }] },
    { group: 'Indian',  units: [
    ...
    { group: 'Weight',  units: [{ label: 'g', grams: 1, step: 5, hint: 'grams' }, { label: 'kg', grams: 1000, step: 0.1, hint: 'kg' }] },
    { group: 'Other',   units: [
  ...
  function _injectUnits() {
    foodList.querySelectorAll('.food-item').forEach(item => {
      ...
      const applyUnit = () => {
        ...
        if (unit.grams === 1 && (srvInput.value === '1' || !srvInput.value)) srvInput.value = 100;
        refreshMacros();
      };
  ...
  foodList.addEventListener('click', e => {
    ...
    setTimeout(() => { unitSel.selectedIndex = 0; srvInput.placeholder = ''; srvInput.step = 0.1; }, 250);
  }, true);
...
(function () {
  ...
  _css.textContent = `
    ...
    .shop-cat-section { margin-bottom:18px; }
    .shop-cat-title {
  ...
  const ING_CATEGORY = {
    ...
    'puffed rice':'🌾 Grains & Dal','vermicelli (semiya)':'🌾 Grains & Dal','vermicelli / rice':'🌾 Grains & Dal',
    'papdi':'🌾 Grains & Dal','sev':'🌾 Grains & Dal',
    ...
    'mosambi':'🍎 Fruits','jackfruit':'🍎 Fruits','coconut':'🍎 Fruits','tamarind':'🍎 Fruits',
    'banana':'🍎 Fruits','orange':'🍎 Fruits','papaya':'🍎 Fruits','grapes':'🍎 Fruits','chikoo':'🍎 Fruits',
    'guava':'🍎 Fruits','mango':'🍎 Fruits','apple':'🍎 Fruits','dates':'🍎 Fruits','lemon':'🍎 Fruits',
    'lime':'🍎 Fruits','kiwi':'🍎 Fruits','pear':'🍎 Fruits',
    ...
    'jaggery':'🌶️ Spices & Condiments','spices':'🌶️ Spices & Condiments','sugar':'🌶️ Spices & Condiments',
    'salt':'🌶️ Spices & Condiments','yeast':'🌶️ Spices & Condiments','basil':'🌶️ Spices & Condiments',
    'oil':'🌶️ Spices & Condiments','red chilli':'🌶️ Spices & Condiments',
  ...
  const CAT_ORDER = [
    '🥬 Vegetables','🌾 Grains & Dal','🍗 Meat & Seafood',
    '🥚 Dairy & Eggs','🍎 Fruits','🌶️ Spices & Condiments','📦 Packaged & Other',];
  ...
  let _ck = {};
  function _loadCk() { try { _ck = JSON.parse(localStorage.getItem('wmp_shop_ck')||'{}'); } catch(e){_ck={};} }
  function _saveCk() { localStorage.setItem('wmp_shop_ck', JSON.stringify(_ck)); }
  ...
  function _renderShopping(container) {
    ...
    container.innerHTML = cats.map(cat =>
      `<div class="shop-cat-section">
        ...
        ${byCat[cat].map(ing => {
          const ck = _ck[ing] ?'checked' : '';
          return `<div class="grocery-item ${ck}">
            <input type="checkbox" ${ck} data-ing="${ing.replace(/"/g,'&quot;')}">
    ...
    container.addEventListener('change', function(e) {
      ...
      if (row) row.classList.toggle('checked', cb.checked);_ck[cb.dataset.ing] = cb.checked;
      _saveCk();
    }, { once: false });
  ...
  function _injectToggle() {
    ...
    bar.innerHTML = `
      <button class="grocery-view-btn active" data-mode="dishes">📋 Dishes</button>
      <button class="grocery-view-btn" data-mode="shopping">🛒 Shopping List</button>
    `;
